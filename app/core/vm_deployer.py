    ...
    def _prepare_all_templates(self, config: dict[str, Any], node_selection: str = None, target_node: str = None) -> bool:
        ...
        try:
            ...
            if node_selection == "specific" and target_node:
                ...
                # Для balanced режима нужно подготовить шаблоны для всех нод
                target_nodes = nodes
            else:
            ...
            for machine_config in config.get('machines', []):
                ...
                # Определяем, на каких целевых нодах нужна эта машина
                nodes_for_this_machine = []
                if node_selection == "specific" and target_node:
                    nodes_for_this_machine = [target_node]
                elif node_selection == "balanced":
                    # Для balanced режима машина может быть размещена на любой ноде
                    nodes_for_this_machine = nodes
                else:
                    # Для других режимов используем первую ноду
                    nodes_for_this_machine = [nodes[0]]
                ...
                for node in nodes_for_this_machine:
                    ...
                    if template_node != node:
                        ...
                        if existing_template_vmid:
                            # Проверяем, существует ли шаблон физически на целевой ноде
                            if self._verify_template_exists(target_node, existing_template_vmid):
